    ...
    <v-stepper v-if="offline==false" v-model="stepper">
      ...
      <v-stepper-items>
              ...
              <v-card elevation="4" height="100%">
                ...
                <v-card-text>
                  <v-container style="overflow-y: scroll;">
                    ...
                    <div v-else>Vous n'avez pas encore d'articles dans votre panier, consultez
                      <NuxtLink to="/catalog"> notre catalogue !</NuxtLink>
                    </div>
                  </v-container>
                </v-card-text>
              </v-card>
                  ...
                  <v-card>
                    ...
                    <v-card-text>
                      <v-row align="center" justify="center">
                        ...
                        <v-col cols="10" v-if="!shopSelected">
        ...
        <v-stepper-content step="2">
          ...
          <div class="mb-12 d-flex flex-column justify-center align-center" height="200px" v-if="!getLoggedIn"
               max-width="300px">
                ...
                <v-col cols="12">
                  <v-card>
                    ...
                    <v-card-text>
                      <v-row>
                        <v-col>
                          Mode de paiements sélectionnés:
                        </v-col>
                        <v-col>
            ...
            <v-col class="d-flex flex justify-center align-center">
              <v-card max-width="450" v-if="this.newOrder!=null">
                ...
                <v-card-text class="flex-column">
                  <p>Vous allez recevoir un e-mail concernant les informations de votre commande ! Vous pouvez aussi
                    consulter à tout moment son état, directement sur
                    <nuxt-link to="/orders/myorders"> page de vos commandes</nuxt-link>
                    .
                  </p>
...
export default {
  ...
  computed: {
    getAddressList() {
      return this.addresses.map(res => {
        return {text: res.adresse, value: res}
      })
    },
      ...
      await this.$axios.$post('/api/logistic/order',
        {
          ...
          contents: this.getArticlesList.map(val => {
            return {code_article: val.code_article, quantité: val.quantity}
          }),
          id_client: this.$auth.user.id_client
        }).then(result => {
        this.newOrder = result.response;
      })
